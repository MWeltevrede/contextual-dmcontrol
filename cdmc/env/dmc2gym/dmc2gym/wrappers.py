...
class ContextualDMCWrapper(gym.Wrapper):
    ...
    def __init__(self, env, physics_seeds, env_kwargs, seed=0):
        gym.Wrapper.__init__(self, env)
        self._max_episode_steps = env._max_episode_steps
        self._env_kwargs = env_kwargs
        self._before_reset_performed = False

        if isinstance(physics_seeds, int):
            self._initial_seed = physics_seeds
            self._i = 0
            self._unbounded_seeds = True
            self._num_physics_seeds = 0
        else:
            self._num_physics_seeds = len(physics_seeds)
            if self._num_physics_seeds > 0:
                self._physics_seeds = physics_seeds
                self._i = 0

                # shuffle the order in which we encounter physics states 
                random.seed(seed)
                self._randomised_seed_indices = np.arange(self._num_physics_seeds)
                random.shuffle(self._randomised_seed_indices)
            self._unbounded_seeds = False
        ...
        if not self._before_reset_performed:
            if self._unbounded_seeds:
                ...
                if self._num_physics_seeds > 0:
                    ...
                    new_physics_seed, _ = self._physics_seeds[self._randomised_seed_indices[self._i]]
    ...
    def reset(self):
        if self._unbounded_seeds:
            ...
            if self._num_physics_seeds > 0:
                self.before_reset()
                # reset environment to reset timestep counters and other things
                _, new_physics_state = self._physics_seeds[self._randomised_seed_indices[self._i]]
                new_physics_state = np.array(new_physics_state)
                self._i = (self._i + 1) % self._num_physics_seeds
                self.env.reset()
