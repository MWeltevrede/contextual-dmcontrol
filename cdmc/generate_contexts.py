...
with Display() as disp:
	...
	for i in range(num_physics_states):
		...
		env.reset()
		physics_states.append((current_seed, env.get_state().tolist()))
	...
	with open(f'{current_file_dir}/../{args.domain_name}_{args.task_name}/train_contexts{num_physics_states}_{args.seed}.json', 'w') as file:
	...
	with open(f'{current_file_dir}/../{args.domain_name}_{args.task_name}/test_contexts{num_physics_states}_{args.seed}.json', 'w') as file:
		json.dump({'states': num_physics_states, 'colors': [dict([(k, v.tolist()) for k,v in color_dict.items()]) for color_dict in test_colors], 'video_paths': test_video_paths}, file)